...
class KhaltiInitPaymentView(APIView):
    """Initiate Khalti payment"""
    permission_classes = [IsAuthenticated]
    
    def post(self, request):
        from .payment_gateway import KhaltiPaymentManager
        from .constants import PLAN_PRICES
        ...
        payment_info = khalti.initiate_payment(
            plan_id=plan,
        ...
        return Response({
            ...
            "api_url": payment_info["api_url"],
            "payload": payment_info["payload"],
            "amount": amount,
            "plan": plan
        })
...
def khalti_verify_payment(request):
    """Verify Khalti payment and activate plan"""
    from .payment_gateway import KhaltiPaymentManager
    ...
    if not all([token, transaction_id, payment_id]):
        return Response(
    ...
    try:
        payment_txn = PaymentTransaction.objects.get(id=payment_id)
    except PaymentTransaction.DoesNotExist:
        return Response(
    ...
    # Verify with Khalti
    khalti = KhaltiPaymentManager()
    is_verified, response_data = khalti.verify_payment(
        token=token,
    ...
    if is_verified:
        ...
        payment_txn.reference_id = response_data.get('idx')
        payment_txn.save()
...
def khalti_callback(request):
    """Khalti callback endpoint - handles payment redirect"""
    token = request.GET.get('token')
    transaction_id = request.GET.get('transaction_id')
    
    if token and transaction_id:
        return redirect(
            f"/billing/khalti-verify/?token={token}&transaction_id={transaction_id}"
        )
