...
import { useCustomers, useCreateInvoice, useCreateCustomer } from "../services/hooks";

const InvoiceCreate = () => {
  const navigate = useNavigate();
  const { customers, loading: customersLoading, refetch: refetchCustomers } = useCustomers();
  const { createInvoice, loading: creatingInvoice, error: createError } = useCreateInvoice();
  ...
  const handleCreateCustomer = async (name) => {
    ...
    if (result) {
      ...
      setCustomerError("Failed to create customer. Please check console for details.");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!customerId && customerNameInput.trim()) {
      console.log('No customer ID, creating customer from input:', customerNameInput.trim());
      const newCustomer = await createCustomer({
        name: customerNameInput.trim(),
      });
      if (!newCustomer) {
        const errMsg = "Failed to create customer. Please try again.";
      ...
      <form onSubmit={handleSubmit} className="max-w-6xl">
        {(createError || customerError) && (
          <div className="mb-6 bg-red-50 border border-red-200 text-red-700 px-6 py-4 rounded-2xl flex items-center gap-3">
            ...
            {customerError || createError?.message}
          </div>
        )}
        ...
        <div className="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm mb-6">
          <h3 className="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 3v2m6-2v2M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2m-4 0V5a2 2 0 114 0v0M9 17H7m8 0H9" />
            </svg>
            Invoice Line Items
          </h3>
